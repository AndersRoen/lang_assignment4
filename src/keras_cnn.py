...
def load_preprocess():
    ...
    return X_train_pad, X_test_pad, y_train, y_test, t, MAX_SEQUENCE_LENGTH, VOCAB_SIZE, EMBED_SIZE, EPOCHS, BATCH_SIZE

def create_compile_mdl(VOCAB_SIZE, EMBED_SIZE, EPOCHS, BATCH_SIZE, MAX_SEQUENCE_LENGTH):
    # create the model
    model = Sequential()
    # embedding layer
...
# train the model
def train_eval_mdl(model, X_train_pad, y_train, EPOCHS, BATCH_SIZE, X_test_pad, y_test, rep_name):
...
def main():
    args = parse_args()
    X_train_pad, X_test_pad, y_train, y_test, t, MAX_SEQUENCE_LENGTH, VOCAB_SIZE, EMBED_SIZE, EPOCHS, BATCH_SIZE = load_preprocess()
    model, EPOCHS, BATCH_SIZE = create_compile_mdl(VOCAB_SIZE, EMBED_SIZE, EPOCHS, BATCH_SIZE, MAX_SEQUENCE_LENGTH)
    report_df = train_eval_mdl(model, X_train_pad, y_train, EPOCHS, BATCH_SIZE, X_test_pad, y_test, args["rep_name"])
